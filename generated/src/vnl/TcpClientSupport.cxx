
// AUTO GENERATED by virtual-network-interface codegen

#include <vnl/TcpClientSupport.hxx>
#include <vnl/Type.hxx>

namespace vnl {

const uint32_t TcpClientBase::VNI_HASH;
const uint32_t TcpClientBase::NUM_FIELDS;

int TcpClientBase::get_field_index(vnl::Hash32 _hash) const {
	switch(_hash) {
		case 0x482df535: return 0;
		case 0x604b2647: return 1;
		case 0xd26001ae: return 2;
		case 0x279e615d: return 3;
		case 0x55f7671e: return 4;
		case 0x1f5839d4: return 5;
		case 0x5e24337a: return 6;
		case 0x2dc3f0d0: return 7;
		case 0x13e7a893: return 8;
		case 0x56a15978: return 9;
		case 0x4bbaccde: return 10;
		case 0xfd6e8f27: return 11;
		case 0x998b692c: return 12;
		default: return -1;
	}
}

const char* TcpClientBase::get_field_name(int _index) const {
	switch(_index) {
		case 0: return "vnl_log_level";
		case 1: return "vnl_msg_timeout";
		case 2: return "vnl_heartbeat_interval";
		case 3: return "send_timeout";
		case 4: return "error_interval";
		case 5: return "are_connected";
		case 6: return "num_drop";
		case 7: return "endpoint";
		case 8: return "port";
		case 9: return "autoclose";
		case 10: return "tcp_nodelay";
		case 11: return "send_buffer_size";
		case 12: return "receive_buffer_size";
		default: return 0;
	}
}

void TcpClientBase::get_field(int _index, vnl::String& _str) const {
	switch(_index) {
		case 0: vnl::to_string(_str, vnl_log_level); break;
		case 1: vnl::to_string(_str, vnl_msg_timeout); break;
		case 2: vnl::to_string(_str, vnl_heartbeat_interval); break;
		case 3: vnl::to_string(_str, send_timeout); break;
		case 4: vnl::to_string(_str, error_interval); break;
		case 5: vnl::to_string(_str, are_connected); break;
		case 6: vnl::to_string(_str, num_drop); break;
		case 7: vnl::to_string(_str, endpoint); break;
		case 8: vnl::to_string(_str, port); break;
		case 9: vnl::to_string(_str, autoclose); break;
		case 10: vnl::to_string(_str, tcp_nodelay); break;
		case 11: vnl::to_string(_str, send_buffer_size); break;
		case 12: vnl::to_string(_str, receive_buffer_size); break;
	}
}

void TcpClientBase::set_field(int _index, const vnl::String& _str) {
	switch(_index) {
		case 0: vnl::from_string(_str, vnl_log_level); break;
		case 1: vnl::from_string(_str, vnl_msg_timeout); break;
		case 2: vnl::from_string(_str, vnl_heartbeat_interval); break;
		case 3: vnl::from_string(_str, send_timeout); break;
		case 4: vnl::from_string(_str, error_interval); break;
		case 5: vnl::from_string(_str, are_connected); break;
		case 6: vnl::from_string(_str, num_drop); break;
		case 7: vnl::from_string(_str, endpoint); break;
		case 8: vnl::from_string(_str, port); break;
		case 9: vnl::from_string(_str, autoclose); break;
		case 10: vnl::from_string(_str, tcp_nodelay); break;
		case 11: vnl::from_string(_str, send_buffer_size); break;
		case 12: vnl::from_string(_str, receive_buffer_size); break;
	}
}

void TcpClientBase::get_field(int _index, vnl::io::TypeOutput& _out) const {
	switch(_index) {
		case 0: vnl::write(_out, vnl_log_level); break;
		case 1: vnl::write(_out, vnl_msg_timeout); break;
		case 2: vnl::write(_out, vnl_heartbeat_interval); break;
		case 3: vnl::write(_out, send_timeout); break;
		case 4: vnl::write(_out, error_interval); break;
		case 5: vnl::write(_out, are_connected); break;
		case 6: vnl::write(_out, num_drop); break;
		case 7: vnl::write(_out, endpoint); break;
		case 8: vnl::write(_out, port); break;
		case 9: vnl::write(_out, autoclose); break;
		case 10: vnl::write(_out, tcp_nodelay); break;
		case 11: vnl::write(_out, send_buffer_size); break;
		case 12: vnl::write(_out, receive_buffer_size); break;
		default: _out.putNull();
	}
}

void TcpClientBase::set_field(int _index, vnl::io::TypeInput& _in) {
	switch(_index) {
		case 0: vnl::read(_in, vnl_log_level); break;
		case 1: vnl::read(_in, vnl_msg_timeout); break;
		case 2: vnl::read(_in, vnl_heartbeat_interval); break;
		case 3: vnl::read(_in, send_timeout); break;
		case 4: vnl::read(_in, error_interval); break;
		case 5: vnl::read(_in, are_connected); break;
		case 6: vnl::read(_in, num_drop); break;
		case 7: vnl::read(_in, endpoint); break;
		case 8: vnl::read(_in, port); break;
		case 9: vnl::read(_in, autoclose); break;
		case 10: vnl::read(_in, tcp_nodelay); break;
		case 11: vnl::read(_in, send_buffer_size); break;
		case 12: vnl::read(_in, receive_buffer_size); break;
	}
}

bool TcpClientBase::vni_call(vnl::io::TypeInput& _in, uint32_t _hash, int _num_args) {
	switch(_hash) {
	case 0x2dc3f0d0: 
		if(_num_args == 1) {
			vnl::read(_in, endpoint);
			return true;
		}
		break;
	case 0x13e7a893: 
		if(_num_args == 1) {
			vnl::read(_in, port);
			return true;
		}
		break;
	case 0x56a15978: 
		if(_num_args == 1) {
			vnl::read(_in, autoclose);
			return true;
		}
		break;
	case 0x4bbaccde: 
		if(_num_args == 1) {
			vnl::read(_in, tcp_nodelay);
			return true;
		}
		break;
	case 0xfd6e8f27: 
		if(_num_args == 1) {
			vnl::read(_in, send_buffer_size);
			return true;
		}
		break;
	case 0x998b692c: 
		if(_num_args == 1) {
			vnl::read(_in, receive_buffer_size);
			return true;
		}
		break;
	case 0x20b6491: 
		switch(_num_args) {
			case 0: {
				if(!_in.error()) {
					reconnect();
					return true;
				}
			}
			break;
		}
		break;
	}
	return Super::vni_call(_in, _hash, _num_args);
}

bool TcpClientBase::vni_const_call(vnl::io::TypeInput& _in, uint32_t _hash, int _num_args, vnl::io::TypeOutput& _out) {
	switch(_hash) {
	case 0x2dc3f0d0: 
		if(_num_args == 0) {
			vnl::write(_out, endpoint);
			return true;
		}
		break;
	case 0x13e7a893: 
		if(_num_args == 0) {
			vnl::write(_out, port);
			return true;
		}
		break;
	case 0x56a15978: 
		if(_num_args == 0) {
			vnl::write(_out, autoclose);
			return true;
		}
		break;
	case 0x4bbaccde: 
		if(_num_args == 0) {
			vnl::write(_out, tcp_nodelay);
			return true;
		}
		break;
	case 0xfd6e8f27: 
		if(_num_args == 0) {
			vnl::write(_out, send_buffer_size);
			return true;
		}
		break;
	case 0x998b692c: 
		if(_num_args == 0) {
			vnl::write(_out, receive_buffer_size);
			return true;
		}
		break;
	}
	return Super::vni_const_call(_in, _hash, _num_args, _out);
}

bool TcpClientBase::handle_switch(vnl::Value* _sample, vnl::Packet* _packet) {
	switch(_sample->get_vni_hash()) {
	}
	return Super::handle_switch(_sample, _packet);
}

bool TcpClientBase::handle_switch(vnl::Value* _sample, vnl::Basic* _input) {
	switch(_sample->get_vni_hash()) {
	}
	return Super::handle_switch(_sample, _input);
}


} // namespace
